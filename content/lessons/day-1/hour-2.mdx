...
```mermaid
flowchart LR
  html["HTML<br/>โครงสร้างและความหมาย<br/>label / input / button"]
  css["CSS<br/>หน้าตาและ layout<br/>สี / ระยะห่าง / responsive"]
  ts["TypeScript<br/>พฤติกรรมและข้อมูล<br/>validate / submit / state"]
  page["หน้า form ที่ user ใช้งานได้"]
  backend["Backend<br/>validate ซ้ำ / บันทึกข้อมูล"]
  database[("Database<br/>เก็บ issue")]

  html -->|"สร้างส่วนประกอบ"| page
  css -->|"ทำให้อ่านง่าย"| page
  ts -->|"ทำให้โต้ตอบได้"| page
  page -->|"submit data"| backend
  ...
  classDef logic fill:#173b2b,stroke:#73c991,color:#ffffff;
  classDef runtime fill:#252526,stroke:#3c3c3c,color:#ffffff;
  classDef data fill:#2a2d2e,stroke:#cca700,color:#ffffff;

  class html markup;
  class css visual;
  class ts logic;
  class page,backend runtime;
  class database data;
```
