...
1. เห็นปัญหาของการเก็บ form, table, badge, type และ mock data ทั้งหมดไว้ใน `app/page.tsx`
2. แยก UI ออกเป็นไฟล์ component จริงใน `components/`
...
| 12-18 นาที | สร้างโครง `components/`, `types/`, `data/` | Live coding |
| 18-35 นาที | สร้างไฟล์ component จริง: `StatusBadge`, `IssueList`, `IssueForm` | Live coding |
| 35-45 นาที | import component กลับมาใช้ใน `app/page.tsx` | Live coding |
...
ทุกอย่างยังอยู่ใน `app/page.tsx`:
...
```text
status badge JSX  -> components/StatusBadge.tsx
issue table JSX   -> components/IssueList.tsx
issue form JSX    -> components/IssueForm.tsx
type ของ issue    -> types/issue.ts
mock issues       -> data/issues.ts
...
หัวใจของชั่วโมงนี้คือ `components/` ส่วน `types` และ `data` เป็นไฟล์กลางที่ช่วยให้ component import ใช้ได้เป็นระเบียบ
...
สร้าง `types/` ไว้ที่ root project ระดับเดียวกับ `app/`

จากนั้นสร้างไฟล์ `issue.ts` ไว้ข้างใน `types/`

วาง type ที่เคยอยู่ด้านบนของ `app/page.tsx` มาไว้ที่ไฟล์นี้ เพื่อให้ `StatusBadge` และ `IssueList` import ไปใช้ได้
...
สร้าง `data/` ไว้ที่ root project ระดับเดียวกับ `app/`

จากนั้นสร้างไฟล์ `issues.ts` ไว้ข้างใน `data/`
...
สร้าง `components/` ไว้ที่ root project ระดับเดียวกับ `app/`

จากนั้นสร้างไฟล์ `StatusBadge.tsx` ไว้ข้างใน `components/`
...
สร้างไฟล์ `IssueList.tsx` ใน `components/`
...
สร้างไฟล์ `IssueForm.tsx` ใน `components/`
...
app/issues/[id]/page.tsx  -> /issues/001
```
...
สร้าง folder `app/issues/` แล้วสร้างไฟล์ `page.tsx`
...
สร้าง folder `app/issues/new/` แล้วสร้างไฟล์ `page.tsx`
...
สร้าง folder `app/issues/[id]/` แล้วสร้างไฟล์ `page.tsx`

```tsx
import { notFound } from "next/navigation";
import { issues } from "@/data/issues";

// params คือข้อมูลจาก dynamic route เช่น [id]
// ใน Next.js เวอร์ชันนี้ params เป็น Promise จึงต้อง await ก่อนใช้งาน
...
export default async function IssueDetailPage({ params }: IssueDetailPageProps) {
  // ดึง id จาก URL เช่น /issues/001 จะได้ id เป็น "001"
...
วางต่อท้ายไฟล์ `app/globals.css`
