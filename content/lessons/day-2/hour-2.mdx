...
1. ย้ายโครงสร้าง HTML จาก Day 1 เข้า `app/page.tsx` ได้
2. เข้าใจความต่างสำคัญระหว่าง HTML และ TSX เช่น `className`, `htmlFor` และ self-closing tag
3. ย้าย CSS จาก `style.css` เข้า `app/globals.css` ได้
...
| เวลา | หัวข้อ | รูปแบบ |
|---|---|---|
| 0-5 นาที | Recap Hour 1: Next.js project structure | เชื่อมเข้าเนื้อหา |
...
แทนที่ code เดิมทั้งหมดใน `app/page.tsx` ด้วยโครงนี้ก่อน เพื่อให้ทุกคนเริ่มจากไฟล์เดียวกัน
...
เปิด `app/globals.css` แล้ววาง CSS จาก Day 1 ต่อจาก Tailwind import หรือ CSS reset ที่มีอยู่เดิม เช่น:
...
ใน project เวอร์ชันใหม่ `layout.tsx` มักมี font variables จาก Geist และ class บน `<body>` อยู่แล้ว ให้แก้ `lang` เป็น `th` และเก็บ class เดิมไว้

### ตัวอย่างเวอร์ชันใหม่
...
- `lang="th"` บอก browser และ accessibility tool ว่าหน้านี้ใช้ภาษาไทย
- `className` บน `<html>` เก็บ font variables และ utility class ที่ project สร้างมาให้
- `body` เป็นจุดครอบ page ทั้งหมด อย่าลบ `{children}` เพราะ page จะไม่ถูกแสดง
...
## Slide 17: โค้ดสุดท้ายของ `app/page.tsx` หลัง Convert
