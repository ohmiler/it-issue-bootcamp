...
<LessonCallout type="action">เปิด Terminal แล้วตรวจ Version ของ Node.js และ npm</LessonCallout>
...
<LessonCallout type="action">สร้าง Folder ว่างชื่อ `it-issue-nextjs` แยกจาก Static Project ของ Day 1 แล้วเปิด Folder ใหม่นี้ใน VS Code</LessonCallout>
...
<LessonCallout type="action">เปิด Terminal ภายใน `it-issue-nextjs` แล้วเริ่ม Development Server</LessonCallout>
...
ใน App Router ไฟล์ `app/page.tsx` ใช้สร้างหน้า `/` ที่เปิดผ่าน `http://localhost:3000/`

ไฟล์ที่ติดตั้งมาจริงมี Code เริ่มต้นยาวกว่านี้ ตัวอย่างย่อเก็บไว้เฉพาะ `<div>`, `<main>` และ `className` ที่ใช้จัด Layout:
...
`Home` คือ Component ของหน้า `/` ส่วน `{/* ... */}` แทน Code ภายในที่ตัดออกให้สไลด์อ่านง่าย ใน Slide 13 เราจะนำหน้า Project ของเราไปแทน Code เดิมทั้งไฟล์
...
<LessonCallout type="action">แทน Code ทั้งไฟล์ `app/page.tsx` ด้วยหน้าเริ่มต้นของระบบแจ้งปัญหา</LessonCallout>
