...
<LessonCallout type="action">แก้บางส่วนใน `components/IssueBoard.tsx`: นำเข้า Type `IssueStatus` เพิ่มใน Import เดิม และเพิ่มฟังก์ชันตามตำแหน่งด้านล่าง</LessonCallout>
...
<LessonCallout type="action">แก้บางส่วนใน `components/IssueList.tsx`: เพิ่ม `statuses` หลัง Type ของ Props ก่อน Component และรับ `onUpdateStatus` เพิ่มในค่าที่ฟังก์ชันรับเข้ามา</LessonCallout>
...
<LessonCallout type="action">เปิด `/issues` แล้วใช้เครื่องมือนักพัฒนาใน Browser (DevTools) ตรวจทั้ง `/` และ `/issues` ที่ความกว้าง 375px และ 1024px</LessonCallout>
