...
| เวลา | หัวข้อ | รูปแบบ |
|---|---|---|
| 0-5 นาที | Recap Tailwind จากชั่วโมงแรก | เชื่อมเข้าเนื้อหา |
...
- ตรวจและเปิดใช้ Tailwind ที่ `app/globals.css`
- ใช้ utility class พื้นฐาน
- แปลง navigation, header, main, footer และ `IssueForm`
...
Hour 1 เราแปลง `IssueForm` เป็น Tailwind แล้ว ตรวจว่า Wrapper ของชื่อและอีเมลใช้ Responsive Grid:
...
- จอเล็ก Field เรียงแนวตั้ง
- จอตั้งแต่ `md` ขึ้นไป Field เรียงสองคอลัมน์
- ทุก Field ใช้ `grid gap-2` เพื่อเว้นระยะระหว่าง Label กับช่องกรอก

ชั่วโมงนี้ไม่ต้องเปลี่ยน Layout ของ Form ซ้ำ เราจะเพิ่มเพียงข้อความช่วยกรอกข้อมูลใน Slide ถัดไป
...
5. เพิ่ม Helper Text ให้ `IssueForm`
...
- `section` เมื่อทั้ง `IssueForm` และ `IssueList` ใช้ Tailwind Class แล้ว
