...
เก็บ Global Style ที่ยังใช้งานภายใน `@layer base` รวมถึง `main` สำหรับ Route ที่ยังไม่ได้ย้าย โดยตัวอย่างนี้แสดงเพียงบางส่วน:

```css
@import "tailwindcss";

@layer base {
  /* เก็บ *, body และ main เดิมไว้ครบ */
}
```
...
Hour 1 ยังเก็บ Style พื้นฐานของ `main` ไว้ให้ Route อื่น ตอนนี้ให้หน้า `/issues` กำหนด Layout ด้วย Tailwind เพื่อให้ความกว้างตรงกับหน้า Home
