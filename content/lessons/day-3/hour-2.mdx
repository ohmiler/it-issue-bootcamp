...
<LessonCallout type="action">แก้บางส่วนใน `components/IssueList.tsx`: แทน `<h2>` เดิมและคำอธิบายใต้หัวข้อถ้ามี ด้วยโค้ดนี้ก่อนกล่องที่ครอบตาราง</LessonCallout>
...
<LessonCallout type="action">แก้ชั่วคราวใน `app/issues/page.tsx`: ส่งรายการว่าง `[]` ให้ `IssueList` ภายใน `<main>` แล้วเปิด `/issues`</LessonCallout>
...
<LessonCallout type="action">รัน `npm run dev` ในโฟลเดอร์หลักของโปรเจกต์ หากยังไม่ได้รัน แล้วเปิด `/` และ `/issues` ใช้เครื่องมือนักพัฒนาใน Browser (DevTools) ปรับความกว้างและลองลิงก์รายละเอียด</LessonCallout>
