...
## Slide 12: จัดกรอบ Navigation

<LessonCallout type="action">แก้บางส่วนใน `app/layout.tsx`: เปลี่ยน Class ของ `<nav>` และ `<div>` ที่ครอบ Link โดยเก็บ Import `Link`, เมนูเดิม และ `{children}` ไว้</LessonCallout>
...
<nav className="border-b border-slate-200 bg-white" aria-label="เมนูหลัก">
  <div className="mx-auto flex max-w-5xl flex-wrap gap-2 px-6 py-3">
    {/* เก็บ Link ทั้งสามรายการเดิมไว้ตรงนี้ */}
  </div>
</nav>
```
...
## Slide 13: ตกแต่ง Link ใน Navigation

<LessonCallout type="action">แก้บางส่วนใน `app/layout.tsx`: เพิ่ม Class ให้ Link ทั้งสามรายการภายใน Navigation โดยคงข้อความและ `href` ของแต่ละรายการไว้</LessonCallout>

ตัวอย่าง Link หน้าแรก ใช้ Class ชุดเดียวกันกับ Link `/issues` และ `/issues/new`:

<CodeChange language="tsx" addedLines="2">

```tsx
<Link
  className="rounded-md px-3 py-2 font-semibold text-slate-700 hover:bg-slate-100"
  href="/"
>
  หน้าแรก
</Link>
...
<LessonCallout type="action">แก้บางส่วนใน `app/globals.css`: ลบ Block ของ Navigation, Header, Footer และ `.issue-list` ออกจากภายใน `@layer base`</LessonCallout>

ตัวอย่างย่ออ้างอิงโครงสร้าง CSS ปัจจุบัน ให้ลบ Selector พร้อม Property ทั้ง Block ไม่คัดลอก Comment ไปแทน CSS จริง

<CodeChange language="css" removedLines="1,2,3,4,5,6,7,8">
...
## Slide 21: ลบ Style เดิมของ Field

<LessonCallout type="action">แก้บางส่วนใน `app/globals.css`: ลบ Block ของ Field ต่อไปนี้ภายใน `@layer base` หลังทุกช่องใน `IssueForm.tsx` มี Utility แล้ว</LessonCallout>

ตัวอย่างย่อระบุ Block ที่ต้องลบทั้งก้อน ไม่คัดลอก Comment ไปแทน CSS จริง

<CodeChange language="css" removedLines="1,2,3,4,5,6,7">
...
หลังลบ ช่องกรอกควรยังมีเส้นขอบ ระยะภายใน และ Focus Ring จาก Utility ที่เพิ่มไว้

---

## Slide 22: ลบ Style เดิมของปุ่มและ Form Row

<LessonCallout type="action">แก้บางส่วนใน `app/globals.css`: ลบ `button` ทั้ง Block รวม `&:hover` ที่ซ้อนอยู่ แล้วลบ `@media` ที่มี `.form-row`</LessonCallout>

ตัวอย่างย่ออ้างอิง Nested CSS ปัจจุบัน ให้ลบทั้งสอง Block ไม่คัดลอก Comment ไปแทน CSS จริง
...
@media (min-width: 720px) {
  .form-row { /* Property เดิม */ }
}
```

</CodeChange>

Media Query นี้มีเพียง `.form-row` จึงลบทั้ง `@media` ได้ หลังลบปุ่มควรยังมีสี Hover และ Focus Ring จาก Tailwind
