...
| Day 3 | ปรับครบแล้ว รอตรวจร่วมกัน | ตรวจ indentation ทั้ง 4 ชั่วโมงแล้ว; Hour 1 มี 23 slides และขั้นลบ CSS อ้างอิง Nested CSS จริง โดยลบ `.issue-list` เมื่อ `main` ใช้ `gap-6` และเก็บ `main` ให้ Route ที่ยังไม่ย้าย; Hour 2 มี 16 slides แยกขั้นย้าย Badge/Table, ลบ CSS หลังย้ายครบ และทดสอบ Empty State ที่ `/issues`; Hour 3 มี 23 slides แยก FormData/Validation/Callback ทีละขั้น ใช้ UUID เต็ม และแจ้งข้อความโดยไม่ล้าง Form เมื่อ `/issues/new` ไม่มี Callback; Hour 4 มี 16 slides แยก Immutable Update, Optional Callback และปุ่มสถานะ พร้อมตรวจ Home แบบแก้ไขกับ `/issues` แบบอ่านอย่างเดียว |
...
- Day 3 ตรวจภาษาครบทั้ง 4 ชั่วโมงแล้ว: อธิบายศัพท์ใหม่ใกล้จุดเริ่มใช้ และใช้คำไทยในคำสั่งทั่วไป โดยคงโค้ดและลำดับการลงมือทำเดิม
